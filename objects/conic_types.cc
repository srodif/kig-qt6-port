...
QList<KLazyLocalizedString> ConicRadicalType::specialActions() const
{
    QList<KLazyLocalizedString> ret;
    ret << kli18n("Switch Radical Lines");
    return ret;
}
...
void ConicRadicalType::executeAction(int i, ObjectHolder &, ObjectTypeCalcer &t, KigPart &d, KigWidget &, NormalMode &) const
{
    ...
    KigCommand *kc = new KigCommand(d, kli18n("Switch Conic Radical Lines").toString());
    mon.finish(kc);
    d.history()->push(kc);
}
