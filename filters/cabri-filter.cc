...
static ObjectTypeCalcer *intersectionPoints(const std::vector<ObjectCalcer *> &parents, int which)
{
    ...
    for (int i = 0; i < 2; ++i) {
        if (parents[i]->imp()->inherits(AbstractLineImp::stype()))
            ++nlines;
        else if (parents[i]->imp()->inherits(CircleImp::stype())) {
            ++ncircles;
            ++nconics;
...
bool KigFilterCabri::supportMime(const QString &mime)
{
    ...
    return (mime == QLatin1String("image/x-xfig")) || (mime == QLatin1String("application/x-cabri"));
}
...
KigDocument *KigFilterCabri::load(const QString &file)
{
    ...
    QRegularExpressionMatch match = header.match(s);
    if (!match.hasMatch()) {
...
#ifdef CABRI_DEBUG
    qDebug() << ">>>>>>>>> version: " << header.capturedTexts().at(3);
    if (!header.capturedTexts().at(13).isEmpty()) {
        qDebug() << ">>>>>>>>> session file:";
        qDebug() << ">>>>>>>>>  -> time = " << header.capturedTexts().at(15) << " sec";
        qDebug() << ">>>>>>>>>  -> action = \"" << header.capturedTexts().at(14) << "\"";
    }
#endif
    if ((major == 1) && (minor == 0)) {
    ...
    while (!f.atEnd()) {
        ...
        args.clear();
        for (std::vector<int>::iterator i = obj->parents.begin(); i != obj->parents.end(); ++i)
        ...
        if (obj->id - priorTo(idsToSkip, obj->id) != calcers.size() + 3)
            KIG_FILTER_PARSE_ERROR;
        oc = nullptr;
#ifdef CABRI_DEBUG
        if (args.size() > 0)
            for (uint j = 0; j < args.size(); ++j) {
                qDebug() << "+++++++++ arg[" << j << "]: " << args[j] << " - " << args[j]->imp()->type()->internalName() << endl;
            }
        else
            qDebug() << "+++++++++ args: NO";
        ...
        if (obj->type == "Pt") {
            ...
            if (args.size() == 1) {
                if (obj->data.size() != 1)
                    KIG_FILTER_PARSE_ERROR;
                ObjectConstCalcer *radc = new ObjectConstCalcer(new DoubleImp(obj->data[0]));
                args.push_back(radc);
                ...
                KIG_FILTER_PARSE_ERROR;
        } else if (obj->type == "Line" || obj->type == "Ray" || obj->type == "Seg" || obj->type == "Vec") {
            if (args.size() == 1) {
                if (obj->data.size() != 2)
                    KIG_FILTER_PARSE_ERROR;
                Coordinate vect(obj->data[0], obj->data[1]);
                ObjectConstCalcer *vectorcalcer = new ObjectConstCalcer(new VectorImp(Coordinate(0, 0), vect));
                args.push_back(vectorcalcer);
                ObjectTypeCalcer *secondpoint = new ObjectTypeCalcer(TranslatedType::instance(), args);
        ...
        } else if (obj->type == "Int" && curVer == CabriNS::CV_1_2) // Cabri1.2
        {
            ...
            if (args.size() == 2
                && (args[1]->imp()->inherits(FilledPolygonImp::stype()) || args[1]->imp()->inherits(FilledPolygonImp::stype3())
                ...
                ObjectCalcer *c = new ObjectTypeCalcer(PolygonSideType::instance(), sideargs);
                c->calc(*ret);
                args.pop_back();
                args.push_back(c);
            } else if (args.size() == 2 && args[1]->imp()->inherits(VectorImp::stype())) {
                ObjectCalcer *c = new ObjectTypeCalcer(LineByVectorType::instance(), args);
                c->calc(*ret);
                args.pop_back();
                args.push_back(c);
            }
            ...
            const ObjectType *t = nullptr;
            if (obj->type == "Perp")
                t = LinePerpendLPType::instance();
            else if (obj->type == "Par")
                t = LineParallelLPType::instance();
            else
                assert(false);
            oc = new ObjectTypeCalcer(t, args);
            ...
            if (args.size() == 2) {
                ObjectCalcer *c = new ObjectTypeCalcer(SegmentABType::instance(), args);
                c->calc(*ret);
                args.clear();
                args.push_back(c);
            }
            if (args.size() == 1
                && (args[0]->imp()->inherits(FilledPolygonImp::stype()) || args[0]->imp()->inherits(FilledPolygonImp::stype3())
        ...
        } else if (obj->type == "Biss" && curVer == CabriNS::CV_1_2) // Cabri1.2
        {
            ...
            for (std::vector<int>::iterator i = obj->incs.begin(); i != obj->incs.end(); ++i)
            ...
            ObjectCalcer *parent = args[0];
            QByteArray prop;
            if ((parent->imp()->inherits(SegmentImp::stype())) || (parent->imp()->inherits(VectorImp::stype())))
                prop = "length";
        ...
        oc->calc(*ret);
#ifdef CABRI_DEBUG
        qDebug() << "+++++++++ oc: " << oc << " - " << oc->imp()->type()->internalName() << endl;
#endif
