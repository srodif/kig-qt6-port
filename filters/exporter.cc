    ...
    , mw(w)
{
    ...
    if (!iconstr.isEmpty())
        setIcon(QIcon(KIconLoader::global()->loadIcon(iconstr, KIconLoader::Desktop, 0, KIconLoader::DefaultState, QStringList(), nullptr)));
...
void KigExportManager::addMenuAction(const KigPart *doc, KigWidget *w, KActionCollection *coll)
{
    ...
    m->setIcon(QIcon(KIconLoader::global()->loadIcon(QLatin1String("document-export"), KIconLoader::Desktop, 0, KIconLoader::DefaultState, QStringList(), nullptr)));
    for (uint i = 0; i < mexporters.size(); ++i)
