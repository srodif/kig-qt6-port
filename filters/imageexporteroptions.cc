    ...
    , minternallysettingstuff(false)
{
    ...
    mxunit = Unit(msize.width(), Unit::pixel, dpiX);
    myunit = Unit(msize.height(), Unit::pixel, dpiY);
...
{
    return QSize((int)qRound(mxunit.getValue(Unit::pixel)), (int)qRound(myunit.getValue(Unit::pixel)));
}
