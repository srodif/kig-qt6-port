...
void CoordinateValidator::fixup(QString &input) const
{
    ...
    if (sc == -1) {
        ...
        switch (mtype) {
        case Polar:
            input.append(QLatin1String(";") + l.positiveSign() + QLatin1String("0"));
            break;
        case Euclidean:
            input.append(QLatin1String(";") + l.positiveSign() + QLatin1String("0") + l.decimalPoint() + QLatin1String("0"));
            break;
        default:
            break;
        }
    };
    QRegularExpressionMatch match = mre.match(input);
...
{
    QRegularExpression r(QString::fromUtf8(CoordinateValidator::reEuclidean));
    QRegularExpressionMatch match = r.match(s);
    ok = match.hasMatch();
    if (ok) {
        QString xs = match.captured(1);
        QString ys = match.captured(2);
        const QLocale currentLocale;
...
void EuclideanCoords::drawGrid(KigPainter &p, bool showgrid, bool showaxes) const
{
    ...
    const int ntick = static_cast<int>(kigMax(hmax - hmin, vmax - vmin) / p.pixelWidth() / 40.) + 1;
    ...
    if (showgrid) {
        ...
        for (double i = hgraphmin; i <= hgraphmax + hd / 2; i += hd)
            p.drawSegment(Coordinate(i, vgraphmin), Coordinate(i, vgraphmax));
        // horizontal lines...
        for (double i = vgraphmin; i <= vgraphmax + vd / 2; i += vd)
            p.drawSegment(Coordinate(hgraphmin, i), Coordinate(hgraphmax, i));
    }
    ...
    if (showaxes) {
        ...
        for (double i = hgraphmin; i <= hgraphmax + hd / 2; i += hd) {
            ...
            p.drawText(Rect(Coordinate(i, 0), hd, -2 * vd).normalized(), currentLocale.toString(i, 'f', hnfrac), Qt::AlignLeft | Qt::AlignTop);
        };
        // y axis...
        for (double i = vgraphmin; i <= vgraphmax + vd / 2; i += vd) {
            if (fabs(i) < 1e-8)
                continue;
            p.drawText(Rect(Coordinate(0, i), 2 * hd, vd).normalized(), currentLocale.toString(i, 'f', vnfrac), Qt::AlignBottom | Qt::AlignLeft);
        };
...
{
    QRegularExpression regexp(QString::fromUtf8(CoordinateValidator::rePolar));
...
void PolarCoords::drawGrid(KigPainter &p, bool showgrid, bool showaxes) const
{
    ...
    if (showaxes) {
        ...
        for (double i = hgraphmin; i <= hgraphmax + hd / 2; i += hd) {
            ...
            p.drawText(Rect(Coordinate(i, 0), hd, -2 * vd).normalized(), is, Qt::AlignLeft | Qt::AlignTop);
        };
        // y axis...
        for (double i = vgraphmin; i <= vgraphmax + vd / 2; i += vd) {
            if (fabs(i) < 1e-8)
            ...
            p.drawText(Rect(Coordinate(0, i), hd, vd).normalized(), is, Qt::AlignBottom | Qt::AlignLeft);
        };
        // arrows on the ends of the axes...
        p.setPen(QPen(Qt::gray, 1, Qt::SolidLine));
        p.setBrush(QBrush(Qt::gray));
...
Coordinate EuclideanCoords::snapToGrid(const Coordinate &c, const KigWidget &w) const
{
    ...
    // the number of intervals we would like to have:
    // we try to have one of them per 40 pixels or so..
    const int ntick = static_cast<int>(kigMax(hmax - hmin, vmax - vmin) / w.pixelWidth() / 40.) + 1;
...
Coordinate PolarCoords::snapToGrid(const Coordinate &c, const KigWidget &w) const
{
...
void PolarCoords::drawGridLine(KigPainter &p, const Coordinate &c, double r) const
{
    ...
    static const iterdata_t iterdata[] = {{+1, +1, &Rect::topRight, &Rect::bottomLeft, 0, M_PI / 2},
