...
NormalModePopupObjects::NormalModePopupObjects(KigPart &part, KigWidget &view, NormalMode &mode, const std::vector<ObjectHolder *> &objs, const QPoint &plc)
    : QMenu(&view)
    , mplc(plc)
    , mpart(part)
    , mview(view)
    ...
    , monlylabels(false)
{
    ...
    static const QString menunames[NumberOfMenus] = {i18n("&Transform"),
                                                     i18n("T&est"),
                                                     i18n("Const&ruct"),
                                                     i18n("&Start"),
    ...
    static const QString menuicons[NumberOfMenus] = {"centralsymmetry",
                                                     "test",
                                                     QString(),
                                                     "system-run",
    ...
    for (uint i = 0; i < NumberOfMenus; ++i) {
        ...
        if (!menuicons[i].isEmpty()) {
            mmenus[i]->setIcon(QIcon(KIconLoader::global()->loadIcon(QLatin1String(menuicons[i]), KIconLoader::Desktop, 0, KIconLoader::DefaultState, QStringList(), nullptr)));
        }
    }
    ...
    connect(this, &QMenu::triggered, this, &NormalModePopupObjects::toplevelMenuSlot);
