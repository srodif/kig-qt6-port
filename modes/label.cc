 ...
 */
void TextLabelModeBase::leftReleased(QMouseEvent *e, KigWidget *v, ObjectTypeCalcer *prevlabel)
{
    switch (d->mwawd) {
    case SelectingLocation: {
        if ((d->plc - e->pos()).manhattanLength() > 4)
            return;
        /*
    ...
    case ReallySelectingArgs: {
         ...
         */
        if (prevlabel && isChild(o->calcer(), prevlabel))
            break;
        QMenu p(v);
        ...
        for (int i = 0; i < l.size(); ++i) {
            ...
            if (iconfile && *iconfile) {
                act = p.addAction(QIcon(mdoc.iconLoader()->loadIcon(QLatin1String(iconfile))), s);
            } else {
                act = p.addAction(s);
            };
...
void TextLabelModeBase::mouseMoved(QMouseEvent *e, KigWidget *w)
{
    if (d->mwawd == ReallySelectingArgs) {
        ...
        for (std::vector<ObjectHolder *>::iterator i = os.begin(); i != os.end(); ++i) {
            if ((*i)->imp()->attachPoint().valid() || (*i)->imp()->inherits(PointImp::stype()) || (*i)->imp()->inherits(CurveImp::stype())) {
                attachable = true;
...
{
    //  QRegularExpression re( QString::fromUtf8( "%[0-9]" ) );
...
void TextLabelModeBase::updateLinksLabel()
{
    ...
    while ((pos = re.match(s, pos).capturedEnd()) != -1) {
        ...
        pos = re.match(s, pos).capturedStart();
        prevpos = pos;
        ++count;
    };
...
void TextLabelConstructionMode::finish(const Coordinate &coord, const QString &s, const argvect &props, bool needframe, ObjectCalcer *locationparent)
{
    std::vector<ObjectCalcer *> args;
    for (argvect::const_iterator i = props.begin(); i != props.end(); ++i)
...
void TextLabelRedefineMode::finish(const Coordinate &coord, const QString &s, const argvect &props, bool needframe, ObjectCalcer *locationparent)
{
    ...
    for (argvect::const_iterator i = props.begin(); i != props.end(); ++i)
        p.push_back(i->get());
    for (std::vector<ObjectCalcer *>::iterator i = p.begin(); i != p.end(); ++i)
        (*i)->calc(mdoc.document());
    ...
    kc->addTask(new ChangeParentsAndTypeTask(mlabel, np, TextType::instance()));
...
{
    if ((mplc - e->pos()).manhattanLength() > 4)
        return;

    bool ok;
    double val = getDoubleFromUser(i18n("Set Value"), i18n("Enter value:"), 0.0, v, &ok, -2147483647, 2147483647, 7);
